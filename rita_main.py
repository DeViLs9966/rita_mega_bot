...
"""
rita_mega_bot.py — Универсальный Rita Mega Bot
Функции:
- Telegram бот с переключением режимов (GPT-4, GPT-2, Google поиск, HuggingFace поиск)
- Генерация программ, игр, читов, apk через GPT-4
- Голосовой ввод (STT) и вывод (TTS)
- OCR (распознавание текста с фото)
- Автообновление кода (только для владельца)
- Приватные команды, защита
- Поддержка файлов, голосовых сообщений
- Все ключи вставлены сразу
"""
...
# === Генерация кода/игр/читов/APK через GPT-4 ===
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with tempfile.NamedTemporaryFile(suffix=".oga") as tf:
        ...
        try:
            ...
            subprocess.run(["ffmpeg", "-i", tf.name, wav_path], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
