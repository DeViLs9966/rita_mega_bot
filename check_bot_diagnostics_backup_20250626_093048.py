...
def read_logs():
    try:
        with open("safe_path_join(logs, rita_bot).log", "r", encoding="utf-8") as f:
            logs = f.read()
...
async def auto_fix_from_logs():
    ...
    try:
        ...
        rita_log_path = Path("safe_path_join(logs, rita_bot).log")
...
LOG_FILE = Path(parent).joinpath("logs", "rita_bot.log")
filename=LOG_FILE,
format="%(asctime)s | %(levelname)s | %(message)s",
level=logging.DEBUG,
encoding="utf-8",
...
def auto_update_script(script_path: Path, repo_raw_url: str) -> bool:
    try:
        ...
        log_info(f"{getattr(script_path, "name", None)} успешно обновлен")
        return True
    except Exception as e:
...
def auto_update_script(script_path: Path, repo_url: str):
    try:
        log_info(f"Попытка автообновления: {getattr(script_path, 'name', str(script_path))}")
        if shutil.which("git") is not None:
            ...
            if safe_path_join(script_dir, ".git").exists():
                ...
                log_info(f"{getattr(script_path, 'name', None)} обновлён через git pull")
                return True
        raw_url = repo_url.rstrip("/") + "/" + getattr(script_path, "name", script_path.name)
        log_debug(f"Скачиваем raw файл по URL: {raw_url}")
        r = requests.get(raw_url, timeout=15)
        if r.status_code == 200:
            script_path.write_text(r.text, encoding="utf-8")
            log_info(f"{getattr(script_path, 'name', None)} обновлён через загрузку raw")
            return True
        else:
    ...
    try:
        ...
        if shutil.which("git") is not None:
            ...
            if git_folder.exists():
                ...
                # Выполняем git pull
                subprocess.run(["git", "-C", str(script_dir), "pull"], check=True)
                log_info(f"{getattr(script_path, 'name', str(script_path))} обновлён через git pull")
                return True
        # Если git отсутствует или нет репозитория — скачиваем raw файл с GitHub
        ...
        if response.status_code == 200:
            ...
            if new_code != current_code:
                script_path.write_text(new_code, encoding="utf-8")
                log_info(f"{getattr(script_path, 'name', str(script_path))} обновлён через загрузку raw файла")
                return True
            else:
                log_info(f"{getattr(script_path, 'name', str(script_path))} уже актуален")
                return False
        else:
            log_error(f"Не удалось скачать raw файл, статус: {response.status_code}")
            return False
    except Exception as e:
        log_error(f"Ошибка автообновления {getattr(script_path, 'name', str(script_path))}: {e}")
        return False
...
def get_script_version(script_path: Path) -> str:
    try:
        ...
        log_error(f"Ошибка получения версии {getattr(script_path, 'name', str(script_path))}: {e}")
        return "error"
async def background_error_log_analysis():
    while True:
        try:
            log_file = Path("safe_path_join(logs, rita_bot).log")  # актуальный путь к логу
            if log_file.exists():
                with open(log_file, "r", encoding="utf-8") as f:
                    log_text = f.read()
            ...
            if "was never closed" in msg or "unexpected EOF" in msg:
                ...
                try:
                    ...
                    with open(script_path, 'w', encoding='utf-8') as f:
                        f.write(fixed_code2)
                    logger.info(f"[FIX] Исправлены незакрытые safe_path_join(скобки, кавычки) в {script_path}")
    ...
    try:
        ...
        if "was never closed" in msg or "unexpected EOF" in msg:
            ...
            try:
                ...
                logger.info(f"[FIX] Исправлены незакрытые safe_path_join(скобки, кавычки) в {script_path}")
                await send_admin_message(f"🛠️ Автофикс: исправлены незакрытые конструкции в {getattr(script_path, "name", None)}")
                return True
            except SyntaxError as e2:
                logger.error(f"[FAIL] Ошибка после попытки исправления: {e2}")
                return False
...
import re
from datetime import datetime
IMPROVEMENT_LOG = Path("safe_path_join(logs, rita_bot).log")
ERROR_LOG_PATH = Path("safe_path_join(logs, rita_bot).log")
...
async def send_admin_detailed_report(context):
    try:
        log_path = Path("safe_path_join(logs, rita_bot).log")
        if not log_path.exists():
            await context.bot.send_message(
                chat_id=OWNER_ID,
...
TELEGRAM_BOT_TOKEN = "7609027838:AAFk2XZRtcvTzbgcrj6QEFWyijon4WsVKj4"
TELEGRAM_ADMIN_ID = 558079551  # твой Telegram ID, число без кавычек
LOG_FILE_PATH = "safe_path_join(logs, rita_bot).log"  # путь к твоему лог файлу
...
def detect_errors_and_successes(log_lines):
    ...
    errors - список строк с safe_path_join(ERROR, Exception)
...
# Настройка логгера
level=logging.INFO
# Ключ OpenAI - вставь свой
OPENAI_API_KEY = "тут_твой_ключ_OpenAI"
openai.api_key = OPENAI_API_KEY
# Пути к скриптам
RITA_MAIN_PATH = Path("rita_main.py")
CHECK_DIAG_PATH = Path("check_bot_diagnostics.py")
LOG_PATH = Path("logs/bot.log")  # Здесь предполагаем, что логи в одном файле
def fix_unclosed_syntax(code: str) -> str:
    """
    Простая попытка закрыть незакрытые конструкции.
    """
...
def remove_duplicate_or_conflicting_code(code1: str, code2: str) -> (str, str):
    """
    Убирает дублирующиеся и конфликтующие блоки кода из двух скриптов,
    оставляя только уникальный и необходимый код.
    Очень простая версия на уровне строк — можно доработать.
    """
    ...
    # Можно дописать более сложную логику фильтрации при необходимости
    return ("\n".join(unique1), "\n".join(unique2))
import re

async def generate_new_functionality(existing_code_main: str, existing_code_diag: str) -> tuple[str, str]:
    """
    Использует OpenAI для генерации новых функций или улучшений
    на основе анализа текущих кодов rita_main.py и check_bot_diagnostics.py.
    Возвращает коды новых функций для каждого скрипта.
    """
    prompt = f"""
Ты - эксперт в разработке Telegram-ботов и систем самоанализа и автообновления.
У тебя есть два скрипта:
rita_main.py:
{existing_code_main[:1500]}...
check_bot_diagnostics.py:
{existing_code_diag[:1500]}...
На основе анализа их текущего состояния и функционала, придумай и создай полезные функции,
которые можно добавить в эти два скрипта для улучшения диагностики, защиты, автообновления и устойчивости.
Требования:
- Отдельно выдай функцию для rita_main.py (начинается с "def new_func_main_...")
- Отдельно выдай функцию для check_bot_diagnostics.py (начинается с "def new_func_diag_...")
Каждую функцию выдай в полном виде, с комментариями.
"""
    logger.info("[INFO] Запрос генерации новых функций в OpenAI...")
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": prompt}],
        ...
        # Безопасное сохранение с utf-8
        if main_func:
            with open("generated_main_function.py", "w", encoding="utf-8") as f:
                f.write(main_func)
        if diag_func:
            with open("generated_diag_function.py", "w", encoding="utf-8") as f:
                f.write(diag_func)
        logger.info("[SUCCESS] Новые функции сохранены.")
        return main_func, diag_func
    except Exception as e:
        logger.error(f"[ERROR] Ошибка генерации новых функций: {e}")
        return "", ""
async def analyze_and_fix_script(script_path: Path, log_path: Path) -> bool:
    try:
        if isinstance(script_path, str):
        ...
        return False
async def main():
    logger.info("[START] Запуск анализа и улучшения скриптов...")
    # Читаем оригинальные коды
    ...
    logger.info("[DONE] Анализ и улучшение завершены.")
